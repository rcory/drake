...
/// mapping) into the output vector (which is mapped using the code's
/// position vector mapping).
            class MapTorqueToPositionVector : public systems::LeafSystem<double> {
            public:
                MapTorqueToPositionVector(MultibodyPlant<double>& plant,
            ...
            void DoMain() {
                ...
                auto instance = multibody::Parser(&plant).AddModelFromFile(full_name);
                ...
                auto remap_sys = builder.AddSystem<MapTorqueToPositionVector>(plant, instance);
                // Add inverse dynamics controller
                auto IDC = builder.AddSystem<systems::controllers::InverseDynamicsController<double>>(plant,
                ...
                builder.Connect(remap_sys->get_output_port(0), plant.get_actuation_input_port());
                ...
                builder.Connect(plant.get_state_output_port(), status_sender.get_state_input_port());
                ...
                // Set initial conditions for system
