...
template <typename T>
void PendulumPlant<T>::dynamics(const PendulumState<T>& state,
                                const PendulumParams<T>& params, T tau,
                                PendulumState<T>* derivative_vector) const {
...
void PendulumPlant<T>::DoCalcTimeDerivatives(
  ...
  dynamics(state, params, get_tau(context), &derivative_vector);
}
