...
/// planar finger and a fixed or planar brick, and the unit tests check the
/// ForceController's ability to regulate a fixed force against the surface of
/// the brick.
class ForceControlTest : public testing::Test {
 protected:
  void BuildDiagram(BrickType brick_type,
    ...
    // This time step matches that of the hardware force controller.
    const double kForceControlStep = 1e-3;

    // This zero order hold breaks an algebraic loop in the force controller,
    // where the output force depends on MBP's contact results, and MBP's
    // `ContactResults` output port depends on the force controller's output.
    auto zoh_contact_results =
        builder.AddSystem<systems::ZeroOrderHold<double>>(
            kForceControlStep, Value<ContactResults<double>>());
    // Connects the "virtual" force sensor to the force controller.
...
// These tests excercise the force controller over a variety of arbitrary
// configurations of a single finger pushing against either a fixed or planar
// (moving) brick. Tests begin with the fingertip initially out of contact with
// the brick, which causes the impedance controller to trigger. Once the finger
// comes into contact with the brick, the controller switches to direct force
// control and attempts to regulate a fixed force.

// The final actual force should be within this percentage of the desired force.
// This value was determined experimentally.
constexpr double kPercentageThreshold = 2e-2;

// Test a single finger pushing against a fixed brick on its positive-z face.
TEST_F(ForceControlTest, PlanarFingerStaticForceControlPosZ) {
  // Build the diagram and simulate the system.
  Vector2d desired_force(-0.03, -0.06);
  ...
  EXPECT_TRUE(CompareMatrices(
      Eigen::Vector2d::Ones(),
      (final_actual_force.array() / desired_force.array()).matrix(),
      kPercentageThreshold));
}

// Test a single finger pushing against a fixed brick on its negative-z face.
...
// Test a single finger pushing against a fixed brick on its positive-y face.
TEST_F(ForceControlTest, PlanarFingerStaticForceControlPosY) {
  // Build the diagram.
  Vector2d desired_force(-0.2, -0.1);
  BuildDiagram(BrickType::FixedBrick,
               Vector3d(0, 0, 0.35) /* brick pose */,
               desired_force, Vector2d(-0.7, 1.5) /* finger q0 */,
...
// Test a single finger pushing against a fixed brick on its positive-y face.
// Same as previoius test, with the elbow bent in the opposite direction.
TEST_F(ForceControlTest, PlanarFingerStaticForceControlPosY2) {
  // Build the diagram.
  Vector2d desired_force(-0.2, -0.05);
  BuildDiagram(BrickType::FixedBrick,
               Vector3d(0, 0.04, 0.1) /* brick pose */,
               desired_force, Vector2d(0.7, -1.5) /* finger q0 */,
               FingerWeldAngle(Finger::kFinger2));
  const Eigen::Vector2d final_actual_force = Simulate();
...
// Test a single finger pushing against a fixed brick on its negative-y face.
TEST_F(ForceControlTest, PlanarFingerStaticForceControlNegY) {
  // Build the diagram.
  Vector2d desired_force(0.2, 0.1);
  BuildDiagram(BrickType::FixedBrick,
               Vector3d(0, 0.04, 0.35) /* brick pose */,
               desired_force, Vector2d(-0.7, 1.5) /* finger q0 */,
...
// Test a single finger pushing against a planar brick moving on a frictional
// floor, where the finger makes contact on the brick's negative-y face.
TEST_F(ForceControlTest, PlanarFingerDynamicForceControlNegY) {
  // Build the diagram.
  Vector2d desired_force(0.057, 0);  // just enough force to overcome stiction.
  BuildDiagram(BrickType::PlanarBrick,
               Vector3d(-0.07, -0.02, 0) /* brick pose */, desired_force,
               Vector2d(-1.5, 1.5),  // finger q0, pose near the center of the
                                     // brick to avoid significant moments
               FingerWeldAngle(Finger::kFinger1));
  const Eigen::Vector2d final_actual_force = Simulate();
  ...
  // time.
  EXPECT_NEAR(1.0, final_actual_force(0) / desired_force(0),
              kPercentageThreshold);
